...
export async function fetchBlockNumberByTimestamp(timestamp: number) {
    const response = await fetchFromEthereumBlocksSubgraph<{
        ...
        `
        {
            blocks (first: 1, orderBy: timestamp, where: { timestamp_gt: ${timestamp}, timestamp_lt: ${timestamp + 600} }) {
                number
            }
        }
    `,
...
export async function fetchBlockNumbersByTimestamps(timestamps: string[]) {
    const queries = timestamps.map((x) => {
        // prettier-ignore
        return `
            t${x}: blocks(first: 1, orderBy: timestamp, orderDirection: desc, where: { timestamp_gt: ${x}, timestamp_lt: ${x + 600} }) {
                number
            }
        `
    })
